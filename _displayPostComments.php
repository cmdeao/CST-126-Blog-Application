...
<!-- 1/31/2021 - Added a confirmation button when deleting a comment. -->
<form action="/commentHandler.php" method="POST" onsubmit="return confirm('Are you sure you want to delete this?');">
<?php if(count($comments) > 0)
{
    $userID = getUserId();
    echo "<h1>Comments:</h1>";    
    echo "<table class='center'>";
    echo "<tr><th>Comment</th><th>Date</th><th>Username</th></tr>";
    for($i=0; $i < count($comments); $i++)
    {
        echo "<tr>";
        echo "<td>" . $comments[$i][0] . "</td>" . "<td>" .
            $comments[$i][1] . "</td>" . "<td>" . $comments[$i][2] . "</td>";
        if($comments[$i][3] == $userID || getUserRole() == 2)
        {
            echo "<td>" . "<button name='deleteComment' type='submit' value=". $comments[$i][4] .">Delete</button></td>";
        }
        echo  "</tr>";
    }
    echo "</table>";
